...
public class A_WrapperTest {
	 ...
	 * ex) 메소드의 매개변수로 객체 타입만이 요구될 때 다형성을 적용하고 싶을 떄 
	 * 	    클래스에서 제공하는 메소드를 사용하고 싶을때 
	 *   	
	 */
	
	//기본 자료형 --> Wrapper 클래스 자료형으로 바꿔보기!
	public void method1() {
		...
		// 1 0 -1 앞이 크면 양수(1) 뒤가 크면 음수(-1) 같으면 0
		...
		//Integer i3 = new Integer(num2); 밑에 와 같이 간단하게 대입을 곧바로 가능 (단, 같은 타입으로만)
		Integer i3 = num2; 
		
		//언제 객체생성을 해서 Boxing합니까??
		//단, 객체 생성을 통해 Boxing 하게 되면 문자열도 넣을 수 있다!! --> 문자열을 숫자화 시켜줌!! (문자열에 숫자만 있어야됨!!)
		Integer i4 = new Integer("123");
		//Integer i4 = "123"; --> 불가능  [위와 같이 문자열을 하고 싶으면 인스턴스를 만들어 줘야함]
		//Integer i4 = new Integer("12황"); --> 문자가 있어서 출력이 안됨 빨간줄은 안 뜨지만.
		...
		//Auto로 해주는 기능들이 많은데 궂이 객체생성을 통해 Boxing을 한다거나 xxxValue 메소드를 통해서 UnBoxing을 하냐?
		//그래서 jdk9버전 이후부터는 해당 기능을 안쓰도록 Deprecated 되어있다. 
