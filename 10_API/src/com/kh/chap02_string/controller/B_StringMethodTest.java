...
public class B_StringMethodTest {
	...
	public void method() {
		...
		//	  문자열에 전달받은 문장열을 하나로 합쳐서 리턴 
		String str2 = str1.concat("!!!"); 
		System.out.println("str2: " + str2);	// new String("Hello World!!!"); [heap 영역]
		
		String str3 = str1 + "!!!";
		System.out.println("str3 : " + str3);	// "Hello World!!!"	[toString 영역]
		...
		// 7. 문자열.substring (int beginIndex) --> 문자열의 beginIndex 위치에서부터 끝까지 문자열을 추출해서 리턴
		// 	    문자열.substring(int beginIndex, int endIndex); --> 문자열의 beginIndex ~ endIndex - 1위치까지의 문자열을 추출해서 리턴
		System.out.println(str1.substring(6));
		System.out.println(str1.substring(0, 6)); // 0 <= ***** <6 ----> 0~5까지 
	
		// 8. 문자열.replace (char old, char new) : String 
		// 	    문자열에서 old 문자를 new문자로 변환한 문자열 리턴 
