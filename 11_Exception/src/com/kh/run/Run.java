...
public class Run {
	...
	public static void main(String[] args) throws IOException{
		...
		/*
		 * *에러종류
		 * 1. 시스템 에러 : 컴퓨터의 오작동으로 발생하는 애러 --> 소스코드 해결안됨 --> 심각한 에러
		 * 2. 컴파일 애러 : 소스코드 문법상 오류 --> 빨간줄로 애초에 알려줌
		 * 3. 런타임 애러 : 코드 상으로는 문제는 없는데 프로그램 실행하고 나서 발생하는 애러  (사용자의 실수)
		 * 
		 * 번외.논리 에러: 문법적으로 문제없고 프로그램 실행할때도 문제는 없지만 프로그램 의도상 맞지 않는 것 
		 * 
		 * 시스템 애러를 제외한 컴퍼일 애러, 런타임 애러 같은 비교적 덜 심각한 것(소스코드로 해결 가능) 들을 작업!
