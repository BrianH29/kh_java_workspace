...
public class A_UnCheckedException {
	...
	public void method1() {
		...
		// 예측 가능한 상황이므로 애초에 예외가 발생 안되게 if문으로 조건 검사하자! => 예외처리  X
		/*
		int result = 0; 
		...
		*/
		...
		//예외 처리 : 예외가 발생했을 떄 처리하는 구문을 작성해놓는것!
		/*
		 * 1. try{} catch() 방법
		 * 
		 * try{
		 * 		예외가 발생될 수 있는 구문; (여러게 기입 가능)
		 * 
		 * } catch (발생될 예외클래스 변수명) {
		...
		try {
			...
			System.out.println("안녕하세요"); // 위쪽에서 예외가 발생했을 경우 실해되지 않음!!
	...
	}/*catch(NegativeArraySIzeException e)*/ // 자식이 부모보다 뒤에 나오면 실해 되지않음. 쓰고 싶으면 부모 앞 RuntimeException 앞에 기입을 해야 사용이 가능하다. WHy? 부모가 크니까 거기 앞에서 다 짜르니까 ㅋ
	System.out.println("종료");
}
	
	/*
	 * *런타임 애러(RuntimeException)
	 * -프로그램 실행시 문제가 발생됨!
	 * -충분히 예측가능한 상황에 있어서는 애초에 예외가 발생 안되게끔 if문으로 핸들링해주는게 권장사항!(예외처리)
	 * -예외처리방법으로도 처리가능하긴 하다!
