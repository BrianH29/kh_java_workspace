...
public class Run {
	...
	// 그말인 즉슨 Object에 있는 메소드를 다 갖다 쓸 수 있다. 
	// 또다른 말로는 Object에 있는 메소드를 재정의 가능(오버라이딩 가능!)
	...
	public static void main(String[] args) {
		...
		// 1. toString() ---------------------------
		// 오버라이딩 전 : Object클래스의 toString() 실행 --> "패키지명 + @ 객체의 해쉬코드 16진수 값" 리턴
		// 오버라이딩 후: Book클래스의 toString() 실행  --> "해당 객체가 가지고 있는 필드값들을 하나의 문자열로" 리턴 
		
		
		System.out.println(bk1.toString());
		System.out.println(bk2.toString());
		// 출력문에서 어떤 레퍼런스를 출력하고자 할 때 JVM이 자동으로 해당 레퍼런스.toString() 호출해준다. 
		// 즉, 래퍼런스.toString()을 명시적으로 제시할 필요 없음!!
