...
public class Run {
	...
	public static void main(String[] args) {
		...
		//기억해둘 것 !! 왼쪽 '=' 오른쪽  왼쪽과 오른쪽 양 옆 자료형(타입)은 같아야 된다. 
		
		//1.부모 타입 레퍼런스로 부모객체 다루는 경우 
		System.out.println("1.부모타입 래퍼런스로 부모 객체 다루는 경우");
		Parent p1 = new Parent();
		p1.printParent();
		//p1 레퍼런스로는 Parent에만 접근 가능 
		...
		//c1 레퍼런스로는 Parent, Child1 둘 다 접근 가능
		
		
		// 지금 부터 다형성 적용되는 내용 
		//3. 부모 타입 레퍼런스로 자식 객체를 다루는 경우 
		System.out.println("3. 부모타입 레퍼런스로 자식 객체 다루는 경우");
		Parent p2 = /*(Parent)*/new Child1();  //  --> 자료형이 양쪽 다르지만 *자동 형변환 실행 
		p2.printParent();
		// p2 레퍼런스로는 Parent에만 접근 가능 
		// 만약 Child1에 접근하고 싶은 강제 형변환을 하면됨!!
		((Child1)p2).printChild1();  // --> 강제 형변환을 통해서 변경 가능 하다.  
...
//		Parent p3 = new Child2();
		...
		// 왜 다형성을 써야되??
		// 만약 Child1객체 2개 필요하고, Child2 객체 2개필요함..
		...
		//((Child2)arr[1]).printChild2(); // 실재로 담겨있는 자식객체의 타입으로 형변환해야만 함!! 
		((Child1)arr[1]).printChild1();
		((Child2)arr[2]).printChild2();
		((Child2)arr[3]).printChild2();
		
		System.out.println("=== 반복문 이용 출력===");
